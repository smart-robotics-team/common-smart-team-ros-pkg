...
class SSC32Driver {
  ...
  private:
	void posCallback(const ssc32_driver::SSC32_Servo::ConstPtr& pos);
...
SSC32Driver::SSC32Driver()
{
	...
	pos_sub = nh.subscribe<ssc32_driver::SSC32_Servo>("TEMERAIRE/ssc32_comm", 5, &SSC32Driver::posCallback, this);
...
void SSC32Driver::posCallback(const ssc32_driver::SSC32_Servo::ConstPtr& pos)
{
	//compute_motor_speed(vel->linear.x, vel->linear.y, vel->angular.z);
	char Serout[500]={0};
	int temp = 0;
